...
function! vim_gtags#job#system_async(cmd, ...) abort
    let l:cmd = a:cmd
    ...
    if s:job_prune(l:cmd) ==# 'run'
        call vim_gtags#echo('The same job is still running')
        return
    end
...
function! s:job_stdout(job_id, data, ...) abort
    if type(a:data) == 1 "string
        call vim_gtags#echo(a:data)
    elseif type(a:data) == 3 "list
        for l:item in a:data
            call vim_gtags#echo(l:item)
        endfor
    endif
endfunction
...
function! s:job_start(cmd, ...) abort
    call vim_gtags#statusline#set('Generating tags in background, please stand by...')
    call vim_gtags#echo(string(a:cmd))
    ...
    elseif has('job')
        ...
        call vim_gtags#echo(system(join(l:cmd)))
        if a:0 != 0
            call a:1()
        endif
...
function! vim_gtags#job#is_running() abort
	if !exists('s:job_list')
		return 0
	endif
	for l:item in s:job_list
